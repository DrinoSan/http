...
void HttpResponse_t::setHeader(const std::string &key, const std::string &value) {
  headers[key] = std::move(value);
}

std::string HttpResponse_t::httpStatusCodeToString(HttpResponse_t::HttpStatusCode code) {
  switch (code) {
